...
# - Rolling-Origin-CV für Lambda-Auswahl
...
source(here("syntax", "03_train_test_split.R"))
...
create_rolling_origin_splits <- function(n, initial_train_size = NULL, validation_size = NULL, n_splits = 5) {
  ...
  if (initial_train_size < 30) {
    stop("initial_train_size ist zu klein für ein stabiles Lasso-Modell.")
  }

  if (initial_train_size + validation_size > n) {
    stop("initial_train_size + validation_size darf n nicht überschreiten.")
  }
...
###############################################################################
# 3. Standardlauf für das Projekt
###############################################################################
...
run_lasso_lag30 <- function(
  ...
  write.csv(
    ...
    here("output", "lasso", "tables", "lasso_run_config.csv"),
    row.names = FALSE
  )
...
if (sys.nframe() == 0) {
  ...
  message("Lasso-Modell erfolgreich geschätzt.")
